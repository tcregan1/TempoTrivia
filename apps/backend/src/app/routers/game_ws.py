...
async def end_round(room_code):
    ...
    await broadcast_room(
        room_code,
        {
            ...
            "payload": {
                "leaderboard": [
                    {"name": p["name"], "score": p["score"]} for p in leaderboard
                ],
...
async def ws_endpoint(ws: WebSocket):
    ...
    try:
        ...
        if len(room_code) != 6 or not room_code.isalnum() or len(nickname) < 2:
            await ws.send_text(
                json.dumps(
                    {"type": "error", "payload": {"code": "INVALID_JOIN"}}
                )
            )
        ...
        rooms[room_code]["players"].append(
            {"id": player_id, "name": nickname, "score": 0}
        )
        ...
        while True:
            ...
            if msg_type == "select_game_mode":
                ...
                if not room or player_id != room.get("host_id"):
                    print(
                        f" Not host! player_id={player_id}, host_id={room.get('host_id')}"
                    )
                    await ws.send_text(
                        json.dumps(
        ...
        if meta:
            ...
            if room:
                ...
                if room.get("host_id") == pid:
                    room["host_id"] = (
                        room["players"][0]["id"] if room["players"] else None
                    )
